...
class Direction
    def initialize(xMod, yMod, direction)
        @xMod = xMod
        @yMod = yMod
        @direction = direction
    end
    ...
    def getNewPosition(currentPosition)
        return [currentPosition[0] + @xMod, currentPosition[1] + @yMod]
    end
end
...
def fillWithOxygen(grid, directions)
    minutes = 0
    openSpaces = grid.select{|key,value| value == '.'}
    while openSpaces.length > 0
        gridPrime = grid.dup
        doneSquares = {}
        grid.each do |key,value|
            if value == '#' || doneSquares[key] != nil
                next
            end
            if value == '*' || value == 'O'
                neighbors = directions.select{|d| grid[d.getNewPosition(key)] == '.'}
                if neighbors.length == 0
                    doneSquares[key] = true
                end
                neighbors.each {|d| gridPrime[d.getNewPosition(key)] = 'O'}
            end
        end
        grid = gridPrime
        openSpaces = grid.select{|key,value| value == '.'}
...
def fillGrid(grid, robotComputer, currentPosition, directions)
    moveDirections = directions.select{|d| grid[d.getNewPosition(currentPosition)] == nil}
    moveDirections.each do |d|
        thisComp = Marshal.load(Marshal.dump(robotComputer))
        thisComp.runUntilOutput(d.direction, true)
        response = thisComp.getOutput
        newPosition = d.getNewPosition(currentPosition)
        if response == 0
            grid[newPosition] = '#'
        elsif response == 1
            grid[newPosition] = '.'
            fillGrid(grid, thisComp, newPosition, directions)
        else 
            # found it
            grid[newPosition] = '*'
            print newPosition
...
def findShortestPath(grid, currentLocation, directions, currentSteps, visited)
    if grid[currentLocation] == '*'
        return [currentSteps]
    end
    returnList = []
    moveDirections = directions.select{|d| grid[d.getNewPosition(currentLocation)] != '#'}
    moveDirections.each do |d|
        newPosition = d.getNewPosition(currentLocation)
        if visited[newPosition] == nil
            visited[newPosition] = true
            returnList.push(findShortestPath(grid, newPosition, directions, currentSteps + 1, visited).flatten)
        end
    end
